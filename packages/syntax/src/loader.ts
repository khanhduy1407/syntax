...
import type { INkdTheme } from './types'
 ...
 * For example:
 * ```ts
 * setCDN('https://unpkg.com/@nkduy/syntax/') // use unpkg
 * setCDN('/assets/syntax/') // serve by yourself
 * ```
 */
...
function _resolvePath(filepath: string) {
  if (isBrowser) {
    if (!CDN_ROOT) {
      console.warn(
        '[NKDuy Syntax] no CDN provider found, use `setCDN()` to specify the CDN for loading the resources before calling `getHighlighter()`'
      )
    }
...
/**
 * @param filepath assert path related to ./packages/syntax
 */
 ...
 */
export async function fetchTheme(themePath: string): Promise<INkdTheme> {
  let theme: IRawTheme = await _fetchJSONAssets(themePath)

  const nkdTheme = toNkdTheme(theme)

  if (nkdTheme.include) {
    const includedTheme = await fetchTheme(join(dirname(themePath), nkdTheme.include))

    if (includedTheme.settings) {
      nkdTheme.settings = includedTheme.settings.concat(nkdTheme.settings)
    }

    if (includedTheme.bg && !nkdTheme.bg) {
      nkdTheme.bg = includedTheme.bg
    }

    if (includedTheme.colors) {
      nkdTheme.colors = { ...includedTheme.colors, ...nkdTheme.colors }
    }

    delete nkdTheme.include
  }
...
export function repairTheme(theme: INkdTheme) {
...
export function toNkdTheme(rawTheme: IRawTheme): INkdTheme {
  const type = (<any>rawTheme).type || 'dark'

  const nkdTheme: INkdTheme = {
    name: rawTheme.name!,
    type,
    ...rawTheme,
  ...
  if ((<any>rawTheme).include) {
    nkdTheme.include = (<any>rawTheme).include
  }
  if ((<any>rawTheme).tokenColors) {
    nkdTheme.settings = (<any>rawTheme).tokenColors
    delete (<any>nkdTheme).tokenColors
  }

  repairTheme(nkdTheme)
